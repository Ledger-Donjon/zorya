...
pub fn handle_bool_and(executor: &mut ConcolicExecutor, instruction: Inst) -> Result<(), String> {
    ...
    // Fetch concolic variables for the two boolean inputs
    ...
    let result_value = input0_var.concolic_and(executor.context, input1_var).map_err(|e| {
        log!(executor.state.logger.clone(), "Error performing AND operation: {}", e);
        e.to_string()
    })?;
    log!(executor.state.logger.clone(), "*** The result of BOOL_AND is: {:?}", result_value.clone());
    ...
    match instruction.output.as_ref().map(|v| &v.var) {
        Some(Var::Unique(id)) => {
            ...
            log!(executor.state.logger.clone(), "Updated unique variable with AND result: {}", unique_name);
        },
        Some(Var::Register(offset, _)) => {
            let mut cpu_state_guard = executor.state.cpu_state.lock().unwrap();
            let concrete_value = if result_value.get_concrete_value() != 0 { 1 } else { 0 };
            cpu_state_guard.set_register_value_by_offset(*offset, concrete_value).map_err(|e| {
        ...
        _ => {
            log!(executor.state.logger.clone(), "Output type is unsupported for BOOL_AND");
...
pub fn handle_bool_negate(executor: &mut ConcolicExecutor, instruction: Inst) -> Result<(), String> {
    ...
    // Fetch concolic variable for input
    log!(executor.state.logger.clone(), "* Fetching instruction.input[0] for BOOL_NEGATE");
    let input_var = executor.varnode_to_concolic(&instruction.inputs[0]).map_err(|e| {
        log!(executor.state.logger.clone(), "Error converting input_var: {}", e);
        e.to_string()
    })?;

    // Perform the negation operation
    let result_value = input_var.concolic_bool_negate().map_err(|e| {
        log!(executor.state.logger.clone(), "Error performing negation: {}", e);
        e.to_string()
    })?;
    log!(executor.state.logger.clone(), "*** The result of BOOL_NEGATE is: {:?}", result_value.clone());
    ...
    match instruction.output.as_ref().map(|v| &v.var) {
        Some(Var::Unique(id)) => {
            ...
            match &result_value {
                ...
                ConcolicEnum::MemoryConcolicValue(memory_var) => {
                    let concolic_var = ConcolicVar::new_concrete_and_symbolic_int(
                        memory_var.get_concrete_value().unwrap_or_default(),
                        &memory_var.symbolic.to_str(),
                        executor.context,
                        64
                    );
                    executor.unique_variables.insert(unique_name.clone(), concolic_var);
        ...
        _ => {
            log!(executor.state.logger.clone(), "Output type is unsupported for BOOL_NEGATE");
            return Err("Output type not supported".to_string());
        }
    }
    ...
    let result_var_name = format!("{}-{:02}-boolnegate", current_addr_hex, executor.instruction_counter);
    match result_value {
