...
mod tests {
    ...
    use zorya::concolic::executor_int::{handle_int_add, handle_int_and, handle_int_equal, handle_int_less, handle_int_sborrow, handle_int_sless, handle_int_sub};
    ...
    fn test_handle_int_sborrow() {
        ...
        let unique_var = executor.unique_variables.get(&unique_name).expect("Unique variable not found");
        assert_eq!(unique_var.concrete.to_u64(), 15); // Ensure it is correctly identifying no overflow
    }
    ...
    fn test_handle_int_and() {
        ...
        let unique_var = executor.unique_variables.get(&unique_name).expect("Unique variable not found");
        assert_eq!(unique_var.concrete.to_u64(), 1); // Ensure the result is correct for AND operation (5 & 3 = 1) because 5 is 101 and 3 is 011
    }
